...
const publishAsync = (
  ...
  setTimeout(() => {
    const registryId = argv.registryId || process.env.GOOGLE_CLOUD_IOT_REGISTRY;
    const payload = `${registryId}/${argv.deviceId}-payload-${messagesSent}`;
    ...
    setTimeout(() => {
      ...
      if (secsFromIssue > argv.tokenExpMins * 60) {
        ...
        connectionArgs.password = createJwt(
          argv.projectId || process.env.GOOGLE_CLOUD_PROJECT,
          argv.privateKeyFile,
          argv.algorithm
        );
...
const detachDevice = (deviceId, client, jwt) => {
  ...
  console.º(`Detaching: ${detachTopic}`);
  let detachPayload = '{}';
  if (jwt && jwt !== '') {
...
const {argv} = require('yargs')
  .options({
    projectId: {
      ...
      description:
        'The Project ID to use. Defaults to the value of the GOOGLE_CLOUD_PROJECT environment variable.',
      requiresArg: true,
      type: 'string',
    },
    cloudRegion: {
      default: process.env.GOOGLE_CLOUD_REGION,
      description: 'GCP cloud region.',
      requiresArg: true,
      type: 'string',
    },
    registryId: {
      default: process.env.GOOGLE_CLOUD_IOT_REGISTRY,
    ...
    deviceId: {
      default: process.env.GOOGLE_CLOUD_IOT_DEVICEID,
  ...
  .example(
    'node $0 mqttDeviceDemo'
  )
  .example(
    'node $0 sendDataFromBoundDevice --projectId=intrepid-snow-322414 \\\n\t--registryId=ebrain-iot-test-registry --deviceId=ebrain-test-device-01 \\\n\t--privateKeyFile=../rsa_private.pem \\\n\t--serverCertFile=../roots.pem --algorithm=RS256 \\\n\t--cloudRegion=europe-west1 --gatewayId=my-node-gateway \\\n'
  )
  .example(
    'node $0 listenForConfigMessages --projectId=intrepid-snow-322414 \\\n\t--registryId=ebrain-iot-test-registry --deviceId=ebrain-test-device-01 \\\n\t--privateKeyFile=../rsa_private.pem \\\n\t--serverCertFile=../roots.pem --algorithm=RS256 \\\n\t--cloudRegion=europe-west1 --gatewayId=my-node-gateway \\\n\t--clientDuration=300000 \\\n'
  )
  .example(
    'node $0 listenForErrorMessages --projectId=intrepid-snow-322414 \\\n\t--registryId=ebrain-iot-test-registry --deviceId=ebrain-test-device-01 \\\n\t--privateKeyFile=../rsa_private.pem \\\n\t--serverCertFile=../roots.pem --algorithm=RS256 \\\n\t--cloudRegion=europe-west1 --gatewayId=my-node-gateway \\\n\t--clientDuration=300000 \\\n'
  )
  .wrap(120)
  .recommendCommands()
